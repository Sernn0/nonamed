...
def build_decoder(
    ...
    # Project to a low-res feature map.
    x = tf.keras.layers.Dense(16 * 16 * 128, use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    x = tf.keras.layers.Reshape((16, 16, 128))(x)

    x = up_block(x, 128, strides=2)  # 32x32
    x = up_block(x, 64, strides=2)   # 64x64
    x = up_block(x, 64, strides=2)   # 128x128
    x = up_block(x, 32, strides=2)   # 256x256
