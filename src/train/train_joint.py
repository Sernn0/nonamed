...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Joint training: style encoder + decoder.")
    parser.add_argument(
        "--train_index",
        type=Path,
    ...
    parser.add_argument("--content_dim", type=int, default=64, help="Content latent dimension.")
    parser.add_argument("--learning_rate", type=float, default=1e-4, help="Learning rate.")
    parser.add_argument("--mae_weight", type=float, default=0.0, help="Optional MAE weight to mix with MSE.")
...
def load_style_encoder(path: Path, style_dim: int) -> keras.Model:
    if path.is_file():
        try:
            print(f"[INFO] Loading style encoder from {path}")
            return keras.models.load_model(path, compile=False)
        except Exception as exc:
            print(f"[WARN] Failed to load style encoder: {exc}. Building new.")
    # TODO: load pretrained backbone when available.
    return build_style_encoder(style_dim=style_dim, input_shape=(256, 256, 1))


def load_decoder(path: Path, content_dim: int, style_dim: int) -> keras.Model:
    model = build_decoder(content_dim=content_dim, style_dim=style_dim, img_shape=(256, 256, 1))
...
class Trainer:
    ...
    def _forward(self, batch: Dict[str, tf.Tensor], training: bool) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        images = batch["image"]
    ...
    def train_step(self, batch: Dict[str, tf.Tensor]) -> tf.Tensor:
        ...
        vars_to_train = self.style_encoder.trainable_variables + self.decoder.trainable_variables
...
def run_training(args: argparse.Namespace) -> None:
    ...
    # Models
    style_encoder = load_style_encoder(args.style_encoder_path, style_dim=args.style_dim)
    decoder = load_decoder(args.decoder_path, content_dim=args.content_dim, style_dim=args.style_dim)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch + 1}/{args.epochs} - loss: {train_loss_val:.4f} - val_loss: {val_loss_val:.4f}")
